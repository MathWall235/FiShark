...
class Level:
    def __init__(self, window, name, menu_option, player_score: list[int]):
        ...
        # Utiliza o valor atualizado de LEVEL_DURATION (em ms) do módulo Const
        self.timeout = Const.LEVEL_DURATION
        pygame.time.set_timer(Const.EVENT_ENEMY, Const.SPAWN_TIME)
        pygame.time.set_timer(Const.EVENT_TIMEOUT, Const.TIMEOUT_STEP)  # Geralmente 100ms
    ...
    def run(self, player_score: list[int]):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                # Chama o método shoot para jogadores e inimigos
                for ent in self.entity_list:
                    if isinstance(ent, Player):
            ...
            self.level_text(18, f'{self.name} - Tempo: {self.timeout / 1000 :.1f}s', Const.C_GREEN, (10, 5))
            self.level_text(18, f'fps: {clock.get_fps() :.0f}', Const.C_DARK_RED, (10, Const.WIN_HEIGHT - 20))
            for ent in self.entity_list:
                if isinstance(ent, Player):
                    self.level_text(18, f'Vida: {ent.health} | Score {ent.score}', Const.C_GREEN, (10, 25))
